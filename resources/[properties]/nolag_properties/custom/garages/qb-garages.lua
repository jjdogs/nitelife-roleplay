...
Config.Functions["OpenGarageMenu"] = {
    ...
    onEnter = function(property)
        ...
        TriggerEvent('qb-garages:client:setHouseGarage', 'Property_' .. tostring(property.id), property:hasKey())
    end,
    onCreate = function (property, point)
        TriggerEvent("qb-garages:client:addHouseGarage", 'Property_' .. tostring(property.id), {
            takeVehicle = {
                x = point.x,
                y = point.y,
                z = point.z,
    ...
    onRemove = function (property)
        TriggerEvent("qb-garages:client:removeHouseGarage", 'Property_' .. tostring(property.id))
    end
}