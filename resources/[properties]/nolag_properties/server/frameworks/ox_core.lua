...
function UpdatePlayerCoords(identifier, coords)
    return MySQL.update.await('UPDATE characters SET x = ?, y = ?, z = ? WHERE stateId = ?', { coords.x, coords.y, coords.z, identifier }) > 0
end
